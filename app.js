...
const fmtKr = v => (Number(v)||0).toLocaleString('da-DK', {minimumFractionDigits:2, maximumFractionDigits:2}) + ' kr';
const fmtNum = v => (Number(v)||0).toLocaleString('da-DK', {minimumFractionDigits:2, maximumFractionDigits:2});
...
let state = {
  ...
  projectName: '',
  globalUnits: 0,
  order: { status:'Tilbud', priority:'Normal', startDate:'', deadline:'', tags:'', notes:'', assignedPrinterIds:[] },
  invoice: { customer:'', addr1:'', addr2:'', invoiceNo:'', terms:'8 dage netto', note:'' },
  settings: { powerPrice:2.5, defaultPrinterWatt:120, moms:25, marginPct:30, switchMin:5, wearPerHour:5, laborRate:250, defaultHoursPerDay:16 },
  filament: [
...
function loadState(){
  ...
  state.order ||= { status:'Tilbud', priority:'Normal', startDate:'', deadline:'', tags:'', notes:'', assignedPrinterIds:[] };
...
function setupEvents(){
  ...
  bindClick('saveBtn', ()=>{ syncTopbarToState(); saveCurrentOrderSnapshot(); saveState(); alert('Ordre gemt'); });
  bindClick('newOrderBtnTop', ()=>createNewOrder(true));
  bindClick('newOrderBtn', ()=>createNewOrder(true));
  bindClick('cloneOrderBtn', cloneOrder);

  bindInput('projectName', ()=>{ syncTopbarToState(); renderHeader(); saveState(); });
  bindInput('globalUnits', ()=>{ state.globalUnits = Math.max(0, Math.floor(num(byId('globalUnits').value))); rerender(); });
  ...
  bindChange('inventoryPadMode', ()=>{ state.app.inventoryPadMode = !!byId('inventoryPadMode').checked; saveState(); renderInventory(); });
...
function syncOrderFieldsToState(){
  state.order.status = byId('orderStatus').value;
  state.order.priority = byId('orderPriority').value;
...
function createNewOrder(confirmPrompt=true){
  ...
  state.projectName = '';
  state.globalUnits = 0;
  state.order = { status:'Tilbud', priority:'Normal', startDate:new Date().toISOString().slice(0,10), deadline:'', tags:'', notes:'', assignedPrinterIds:[] };
  state.invoice = { customer:'', addr1:'', addr2:'', invoiceNo:'', terms:'8 dage netto', note:'' };
  state.items = [];
  state.plateProgress = {};
...
function renderOrderHistory(){
  const tbody = byId('orderHistoryBody'); if(!tbody) return;
  const q = (byId('orderSearch')?.value || '').trim().toLowerCase();
  const sf = byId('orderStatusFilter')?.value || '';
  const rows = state.ordersHistory.filter(o => !sf || o.order?.status===sf).filter(o=>{
    if(!q) return true;
...
window.openOrder = function(id){
  const o = state.ordersHistory.find(x=>x.id===id); if(!o) return;
...
function getAssignedPrinters(){
  return state.printers.filter(p => (state.order.assignedPrinterIds || []).includes(p.id) && p.status === 'Aktiv');
}
...
function renderPrinterAssignments(){
  const box = byId('orderPrinterAssignments'); if(!box) return;
  if(state.printers.length===0){ box.innerHTML = '<span class="text-slate-400 text-xs">Ingen printere oprettet endnu</span>'; return; }
  const ids = state.order.assignedPrinterIds || [];
  box.innerHTML = state.printers.map(p=>`<button type="button" onclick="togglePrinterAssign('${p.id}')" class="px-2 py-1 rounded text-xs border ${ids.includes(p.id)?'bg-sky-700 border-sky-500 text-white':'bg-slate-700 border-slate-600 text-slate-200'}">${esc(p.name)}</button>`).join('');
}
...
function computeCostBreakdown(){
  ...
  return { items, totals:{ totalEx, totalInc, totalPrintPlus: items.reduce((s,i)=>s+i.totalPrintHours,0) } };
}
...
function addItem(){
  ...
  if(!name || weightPlate<=0) return alert('Udfyld navn og vægt');
  state.items.push({
    id: uid(),
    name,
    weightPlate,
...
function clearItemForm(){
  ['itemName','itemWeightPlate','itemPiecesPerPlate','itemMultPerUnit','itemPlateHours','itemPlateMinutes'].forEach(id=>setVal(id,''));
}
window.updateItemField = function(id, field, value){
  const it = state.items.find(x=>x.id===id); if(!it) return;
  if(['weightPlate','piecesPerPlate','multPerUnit','plateHours','plateMinutes'].includes(field)) it[field] = num(value);
  else it[field] = value;
  rerender();
};
...
function populateFilamentSelect(){
  const sel = byId('itemFilamentType'); if(!sel) return;
  sel.innerHTML = state.filament.map(f=>`<option>${esc(f.name)}</option>`).join('');
}
function renderItems(){
  const tbody = byId('itemsBody'); if(!tbody) return;
...
window.updateFilField = function(id, field, value){
  const f = state.filament.find(x=>x.id===id); if(!f) return;
  if(['price','stockKg'].includes(field)) f[field] = num(value); else f[field] = value;
  rerender();
};
...
function renderFilament(){
  const tbody = byId('filamentBody'); if(!tbody) return;
  const usage = {};
  state.items.forEach(it=>{ const d = itemDerived(it); usage[it.filament] = (usage[it.filament] || 0) + d.filamentKg; });
  tbody.innerHTML = state.filament.map(f=>`<tr>
...
window.updatePartField = function(id, field, value){
  ...
  if(['price','qtyPerUnit','stock'].includes(field)) p[field] = num(value); else p[field] = value;
  rerender();
};
...
function renderInventory(){
  ...
  const tbody = byId('partsBody'); if(!tbody) return;
  const units = Math.max(0, num(state.globalUnits));
...
function addPrinter(){
  ...
  state.printers.push({
    ...
    status: byId('printerStatus').value,
...
window.updatePrinterField = function(id, field, value){
  ...
  if(['watt','hoursPerDay','liveWatt'].includes(field)) p[field] = num(value); else p[field] = value;
  rerender();
};
...
function renderPrinters(){
  const tbody = byId('printersBody'); if(!tbody) return;
  tbody.innerHTML = state.printers.map(p=>`<tr>
...
function renderCustomers(){
  const tbody = byId('customersBody'); if(!tbody) return;
...
function renderCalendar(){
  ...
  document.querySelectorAll('.modeBtn').forEach(btn=>btn.classList.toggle('bg-indigo-600', btn.dataset.mode===calendarMode));
  const mount = byId('calendarMount'); if(!mount) return;
  ...
  if(calendarMode==='day'){
    ...
    `).join('') || '<div class="text-slate-400">Ingen booking endnu</div>';
    ...
    mount.innerHTML = Object.keys(weeks).sort().map(weekStart=>{
      ...
      return `<div class="mb-4"><div class="font-semibold mb-2">Uge fra ${weekStart}</div><table class="table-ui"><thead><tr><th>Printer</th><th class="text-right">Timer</th><th>Ordrenumre</th></tr></thead><tbody>
      ${Object.keys(byPrinter).map(pr=>`<tr><td>${esc(pr)}</td><td class="text-right">${fmtNum(byPrinter[pr])} t</td><td>${[...new Set(rows.filter(r=>r.printerName===pr).map(r=>r.orderNo))].join(', ')}</td></tr>`).join('')}
      </tbody></table></div>`;
    }).join('') || '<div class="text-slate-400">Ingen booking endnu</div>';
  } else {
    const months = {};
    ...
    mount.innerHTML = Object.keys(months).sort().map(month=>{
      ...
      return `<div class="mb-4"><div class="font-semibold mb-2">${month}</div><table class="table-ui"><thead><tr><th>Dato</th><th>Printere der kører</th><th class="text-right">Timer total</th></tr></thead><tbody>
      ${Object.keys(byDate).sort().map(date=>`<tr><td>${date}</td><td>${[...new Set(byDate[date].map(r=>r.printerName))].join(', ')}</td><td class="text-right">${fmtNum(byDate[date].reduce((s,r)=>s+r.hours,0))} t</td></tr>`).join('')}
      </tbody></table></div>`;
    }).join('') || '<div class="text-slate-400">Ingen booking endnu</div>';
  }
}
...
function renderDashboard(){
  ...
  if(byId('dashOverview')) byId('dashOverview').innerHTML = `
  ...
  const usage = {};
  state.items.forEach(it=>{ const d = itemDerived(it); usage[it.filament] = (usage[it.filament] || 0) + d.filamentKg; });
  state.filament.forEach(f=>{ const miss = Math.max(0, (usage[f.name] || 0)-num(f.stockKg)); if(miss>0) alerts.push(`Mangler ${fmtNum(miss)} kg ${f.name}`); });
  const units = Math.max(0, num(state.globalUnits));
  state.parts.forEach(p=>{ const miss = Math.max(0, units*num(p.qtyPerUnit)-num(p.stock)); if(miss>0) alerts.push(`Mangler ${miss} stk ${p.name}`); });
...
function renderInvoice(){
  ...
  const units = Math.max(1, num(state.globalUnits) || 1);
  ...
  if(byId('invDate')) byId('invDate').textContent = new Date().toLocaleDateString('da-DK');
  ...
  const tbody = byId('invoiceBody'); if(tbody){
    tbody.innerHTML = lines.map(l=>`<tr><td>${esc(l.desc)}</td><td class="text-right">${l.qty}</td><td class="text-right">${fmtKr(l.unitEx)}</td><td class="text-right">${fmtKr(l.unitInc)}</td><td class="text-right">${fmtKr(l.totalInc)}</td></tr>`).join('');
  }
...
function exportCSV(){
  ...
  cb.items.forEach(d=>rows.push([state.orderNo, state.projectName, d.name, d.totalPieces, d.plates, d.totalPrintHours.toFixed(2), d.filamentKg.toFixed(3), d.matEx.toFixed(2), d.status]));
